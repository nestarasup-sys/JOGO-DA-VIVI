...
import type {DecisionLog,Effects,HistoryLine,Story} from '../game/types';
import rawStory from '../data/season1.json';
import {storySchema} from '../game/schema';

const clamp=(n:number,min:number,max:number)=>Math.max(min,Math.min(max,n));
export type GameState={
 name:string;coins:number;episode:number;unlockedEpisode:number;completed:number[];
 affinity:Record<string,number>;traits:Record<string,number>;route:string|null;flags:Record<string,unknown>;
...
export const useGame=create<GameState>()(persist((set,get)=>({...initial,
 ...
 applyEffects:(e={})=>set(s=>{
   const affinity={...s.affinity};
   Object.entries(e.affinity||{}).forEach(([k,v])=>affinity[k]=clamp((affinity[k]||0)+(v||0),0,100));
   const traits={...s.traits};
   Object.entries(e.traits||{}).forEach(([k,v])=>traits[k]=clamp((traits[k]||0)+(v||0),0,100));
   return {
    affinity,traits,flags:{...s.flags,...(e.flags||{})},coins:Math.max(0,s.coins+(e.coins||0)),
    route:e.route??s.route,messages:[...new Set([...s.messages,...(e.messages||[])])],
    unlockedCG:[...new Set([...s.unlockedCG,...(e.unlockCG?[e.unlockCG]:[])])],
    ownedOutfits:[...new Set([...s.ownedOutfits,...(e.outfit?[e.outfit]:[])])],equipped:e.outfit??s.equipped
   }
 }),
 completeEpisode:(ep)=>set(s=>({
   completed:[...new Set([...s.completed,ep])],
   unlockedEpisode:Math.max(s.unlockedEpisode,Math.min(s.story.episodes.length,ep+1)),
   episode:Math.max(s.episode,Math.min(s.story.episodes.length,ep+1)),playerOpen:false,autoMode:false
 })),
 buyOutfit:(id,price)=>{
   const s=get();if(s.ownedOutfits.includes(id)){set({equipped:id});return true}
   if(s.coins<price)return false;
   set({coins:s.coins-price,ownedOutfits:[...s.ownedOutfits,id],equipped:id});return true
 },
 visit:(id)=>set(s=>s.visited.includes(id)?{}:{visited:[...s.visited,id],coins:s.coins+15}),
 finishEvent:(id,reward,aff)=>set(s=>{
   if(s.eventDone.includes(id))return {};
   const affinity={...s.affinity};if(aff)affinity[aff]=clamp((affinity[aff]||0)+6,0,100);
   return {eventDone:[...s.eventDone,id],coins:s.coins+reward,affinity}
 }),
 setStory:(story)=>{storySchema.parse(story);set({story})},
 resetStory:()=>set({story:rawStory as Story}),
